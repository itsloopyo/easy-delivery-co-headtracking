...
namespace EasyDeliveryCoHeadTracking.Tests
{
    /// <summary>
    ...
    public class ConfigTests
    {
        ...
        {
            byte[] rendered = EasyDeliveryCoConfig.Table().RenderFresh(new RenderHeader(EasyDeliveryCoConfig.DisplayName));
        ...
        {
            var config = new EasyDeliveryCoConfig();
        ...
        public void FirstLaunchCreatesTheCommittedBytes()
        {
            ...
            {
                ConfigLoadResult<EasyDeliveryCoConfig> loaded = Owner(dir.Path).Load();
        ...
        public void DefaultRowsFollowDefaultsIni()
        {
            using (var dir = new TempDir())
            {
                ...
                ConfigLoadResult<EasyDeliveryCoConfig> next = Owner(dir.Path).Load();
        ...
        public void TheYawToggleSavesOnlyItsRowAndItComesBack()
        {
            using (var dir = new TempDir())
            {
                ...
                ConfigOwner<EasyDeliveryCoConfig> owner = Owner(dir.Path);
                owner.Load();
        ...
        private static ConfigOwner<EasyDeliveryCoConfig> Owner(string dir)
        {
        ...
        private sealed class TempDir : IDisposable
        {
            public TempDir()
            {
                Path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "edc-config-" + Guid.NewGuid().ToString("N"));
